...
const signIn = async (data) => {
    ...
    if(compareResult) {
        const accessToken = await createToken({username: result.username}, accessTokenKey, 60*10);
        const refreshToken = await createToken({username: result.username}, refreshTokenKey, 60*60);
        ...
        const userInfo = await PersonController.updatePerson(username, { refresh_token: refreshToken })
        if (userInfo) {
            return {
                success: true,
                tokens: {
...
const refreshHandler = async (token) => {
    const [result] = await PersonController.findPerson({ refresh_token: token });
    if ((result) && (result.username)) {
        const decodeToken = await validateToken(token, refreshTokenKey);
        const accessToken = await createToken({username: decodeToken.username}, accessTokenKey, 60*10);
...
module.exports = { signUp, signIn, activateHandler, refreshHandler, logoutHandler }
