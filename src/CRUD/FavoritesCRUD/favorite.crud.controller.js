const { createFavoriteBookH, getFavoriteBooksH, deleteFavoriteBookH } = require('./favorite.crud.handler');
...
const deleteFavoriteBook = async (req, res) => {
    const result = await deleteFavoriteBookH(req.query.username, req.query.title)
    res.status(200).json(result);
    res.end();
}
